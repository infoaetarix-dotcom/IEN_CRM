...
import { uploadStudentDocument } from '@/app/(public)/upload/actions';
...
/**
 * Add-only dropzone for the public /upload/[token] page — no download, no
 * delete, nothing else about the application exposed. Appends optimistically
 * on a successful upload rather than a full router.refresh(), since this
 * page has no session to re-fetch against.
 */
export function StudentUploadForm({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    start(async () => {
      const res = await uploadStudentDocument(token, formData);
      if (!res.ok) {
